...
export class BalotariosService {
  ...
  async create(createBalotarioDto: CreateBalotarioDto): Promise<BalotarioDto> {
    ...
    await this.especialidadesService.findOne(createBalotarioDto.especialidad.id);
  ...
  async findAll(filterDto: FilterBalotarioDto): Promise<PaginatedResponseDto<BalotarioDto>> {
    const { nombre, institucion, anio, especialidadId, activo, sortBy, order, page, limit } =
      filterDto;
  ...
  async update(id: string, updateBalotarioDto: UpdateBalotarioDto): Promise<BalotarioDto> {
    ...
    if (updateBalotarioDto.especialidad?.id &&
      (!balotario.especialidad || updateBalotarioDto.especialidad.id !== balotario.especialidad.id)
    ) {
      await this.especialidadesService.findOne(updateBalotarioDto.especialidad.id);
    }
